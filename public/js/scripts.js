...
---------------------------------------------------*/	
	...
	function FirstLoad() {
		...
		var windowWidth = $(document).find('.img-mask').width()
		var imgSelector
		...
		$('.section').each(function() {
			...
			var image = $(this).find('.section-image').data(''+imgSelector+'');	
			...
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					...
					setTimeout( function(){
						...
						TweenMax.set($("#main-content, .fp-tableCell"), {y: 200});
					} , 500 );					
					//Fade In Navigation Lists
					var tl = new TimelineLite();
		...
		//Logo Effect
			...
			if (stringPathName != '/'){
				$(".black-logo").removeClass('active-item');
			}
			...
			$('#logo').click(function(e){
				...
				$('.logo_container').fadeToggle('slow');
				if (width < 767){
					if ($('#logo').hasClass("open")) {
						TweenMax.to($(".menu-info"), 0.4, {force3D:true, opacity:0, x: 1000,ease:Power2.easeInOut  });
					}
					else{
						TweenMax.to($("#hero, .menu-info, .next-project-info"), 0.4, {force3D:true, opacity:1, x: 0, delay:0.4, ease:Power3.easeOut});
					}
				}
		  ...
		  $(window).on('popstate', function() {
			...
			section.load(url+' .cd-main-content > *', function(event){
			  ...
			  setTimeout(function(){
				...
				//ga('set', 'page', '/' + url);
				//ga('send', 'pageview');
				console.log('Next URL: '+window.location.href);
...
---------------------------------------------------*/
	...
	function ShowCookiesUse(){
		if( activePage == 'main' ) {
            var $container = $('.pushNotificationContainer')
            var link = "{{ route('privacy.policy') }}"
            $container.css('background', 'none')

            $container.append('<div class="cookies"><p>We want to make our website more user-friendly and continuously improve it.</p><p> If you continue to use the website, you agree to the use of cookies.</p><p>For more information, please refer to our <a data-type="page-transition"  href="https://vocsway.com/privacy-policy">Cookie Statement.</p></div>')
...
---------------------------------------------------*/	
	...
	function RunJS(objID) {
		...
		var script = ''
		objID = objID.replace("https://vocsway.com/", "");
		var count = objID.search("/")
	...
	function AppendScript(script){
        ...
        $(document).find('#ajaxScript').remove()
        console.log('SCRIPT LOADED: '+script + ' / on page - '+ activePage)
        s.id = 'ajaxScript'
        s.type = "text/javascript";
        s.src= '/js/'+script+'.js';
        eval(document.getElementsByTagName("head")[0].appendChild(s))
	}
...
---------------------------------------------------*/
	...
	function FullPageDestroy() {
		...
		if( $('#showcase-slider').length > 0 ){	
			...
			//$('.nav-item-footer').html('');
			//$('.nav-item-footer').html($('.nav-item-footer').html());
...
---------------------------------------------------*/
	...
	function FullPage() {
		...
		if( $('#showcase-slider').length > 0 ){	
			...
			var pageItem = $('.page-item-footer');
			var itemCount = $('.page-item').length;
			
			if(itemCount > 0){
							
				var firstPage = $('.page-item-footer').find('#fp-viewing-1');
				firstPage.addClass('active');

				var scrollVar = firstPage.index();
			...
			$('#showcase-slider').fullpage({
				...
				onLeave: function(index, nextIndex, direction){					
					...
					if(itemCount > 0){
						...
						var prevPage = $('.page-item-footer').find('#fp-viewing-'+prevItem);
					...
					if(index  && direction ==='down'){
						...
						if(itemCount > 0){
							var nextPage = $('.page-item-footer').find('#fp-viewing-'+nextIndex);
							nextPage.removeClass('down');
							nextPage.addClass('active');
							$('.page-item-footer').find('#fp-viewing-'+index).addClass('up');
							var scrollVar = nextPage.index();
							$('#scroll_page').animate({
								scrollTop: (scrollVar * 16) - 7
							}, 777);
						}
						$(this).trigger('mouse-scrolled-down');
						...
						if(itemCount > 0){
							var nextPage = $('.page-item-footer').find('#fp-viewing-'+nextIndex);
							$('.page-item-footer').find('#fp-viewing-'+index).addClass('down');
							nextPage.removeClass('up');
							nextPage.addClass('active');
							var scrollVar = nextPage.index();
							$('#scroll_page').animate({
								scrollTop: scrollVar*16 - 7
							}, 777);
							$(this).trigger('mouse-scrolled-up');
						}
			...
			$(window).on( 'resize', function () {
				...
				var attr_col = $('#portfolio').attr('data-col');
				 ...
				 if (winWidth >= 1466) {
					...
					postHeight = postWidth/16*9
					$container.find('.item').each(function () { 
						
						if ( $('#portfolio').attr('data-col') === '3' ) {
							$('.item').css( { 
								width : postWidth - 20 + 'px',
								height : postWidth / 16 * 9   + 'px',
								margin : 10 + 'px',
							});
						} 
			...
			$('.has-animation').each(function() {	
				$(this).appear(function() {				
					$(this).delay($(this).attr('data-delay')).queue(function(next){
						$(this).addClass('animate-in').trigger('inView');
						next();
					});				 		
				});		
			});
